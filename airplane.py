...
data2=data_loader(r'C:\Users\29915\Desktop\final3\Implicit_polynomial\CNN\code\airpane\airplane0.csv')
...
plt.figure()
plt.scatter(data2[:,0],data2[:,1],color='red',s=0.5)
plt.xlim(-3,3)
plt.ylim(-3,3)
plt.show()
...
plt.figure()
plt.scatter(data1[:,0],data1[:,1],color='blue',s=0.5)
plt.scatter(data2[:,0],data2[:,1],color='red',s=0.5)
plt.scatter(data3[:,0],data3[:,1],color='green',s=0.5)
plt.xlim(-3,3)
plt.ylim(-3,3)
plt.show()
...
class IPEncoder(torch.nn.Module):
    def __init__(self,n_hidden1,n_hidden2,n_hidden3,n_hidden4,n_hidden5,n_hidden6,n_hidden7,n_hidden8,n_hidden9,n_hidden10,n_hidden11,n_hidden12,n_hidden13,n_hidden14,n_hidden15):
        ...
        self.weights1_1=Parameter(torch.rand([self.n_hidden1,self.n_hidden2]))
        self.weights1_2=Parameter(torch.rand([self.n_hidden1,self.n_hidden2]))
        ...
        self.weights12=Parameter(torch.rand([self.n_hidden12,self.n_hidden13]))
        self.weights13=Parameter(torch.rand([self.n_hidden13,self.n_hidden14]))
        self.weights14=Parameter(torch.rand([self.n_hidden14,self.n_hidden15]))

        
        self.bias1_1=Parameter(torch.rand([self.n_hidden2]))
        self.bias1_2=Parameter(torch.rand([self.n_hidden2]))
        ...
        self.bias12=Parameter(torch.rand([self.n_hidden13]))
        self.bias13=Parameter(torch.rand([self.n_hidden14]))
        self.bias14=Parameter(torch.rand([self.n_hidden15]))
...
scheduler1=lr_scheduler.StepLR(optimizer1,step_size=500,gamma=0.8)
...
target=torch.cat([target,target3],0)
...
x,y=symbols('x y')
plot_implicit(parse_expr(expression),(x,-3,3),(y,-3,3))