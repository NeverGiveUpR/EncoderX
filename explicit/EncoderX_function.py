...
def GenerateData1D(start,end,input_size):
...
data1,target1=GenerateData1D(-4,4,input_size)

plt.scatter(data1,target1,color='blue',marker='.',s=0.5)
plt.legend(['y=0.3exp(-x)sin(x)'],loc='best',fontsize=12,markerscale=5)
plt.show()
...
plt.legend(['y=0.3exp(-x)sin(x)','Encoder X'],loc='best',fontsize=12,markerscale=5)
plt.show()
