...
def GenerateData(input_size):
        num_input=input_size[0]
        dim=input_size[1]
        miu=np.array([0.25,0.25])
        _lambda=np.array([1.0,0.25])
        yita=np.array([[0.0,0.5],[0.0,0.0]])
        w1=np.array([-0.5])
        w2=np.array([1])
        
        data=np.random.uniform(-1, 1, (num_input,dim))
        A=np.diag(np.exp(_lambda))+np.triu(yita, k=1)
        Sigma=np.matmul(A.transpose(),A)
        data_submean=data-miu
        dist=w1*np.matmul(data_submean,np.matmul(Sigma,data_submean.transpose())).diagonal()
        target=w2*np.exp(dist)

        return data,target

data1,target1=GenerateData(input_size)  
...
while True:
    ...
    print('|Epoch:',t,'|Loss:',loss1.data.numpy())
